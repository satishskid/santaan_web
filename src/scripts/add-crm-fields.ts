...
async function addCRMFields() {
    ...
    const columns = [
        ...
        { name: 'owner_name', sql: 'ALTER TABLE contacts ADD COLUMN owner_name TEXT' },
        { name: 'owner_email', sql: 'ALTER TABLE contacts ADD COLUMN owner_email TEXT' },
        { name: 'next_follow_up_at', sql: 'ALTER TABLE contacts ADD COLUMN next_follow_up_at TEXT' },
