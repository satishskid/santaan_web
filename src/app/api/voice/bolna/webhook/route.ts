...
import {
  NormalizedBolnaPayload,
  normalizeBolnaPayload,
  voiceEducationLink,
} from "@/lib/voice-ai";
...
const BOLNA_TERMINAL_STATUSES = new Set([
  "completed",
  "failed",
  "no-answer",
  "busy",
...
function buildVoiceNote(lead: NormalizedBolnaPayload, center: string) {
  return [
...
function isTerminalBolnaStatus(status: string) {
  return BOLNA_TERMINAL_STATUSES.has(String(status || "").trim().toLowerCase());
}

function voicePostCallParamValue(token: string, lead: NormalizedBolnaPayload) {
...
function buildVoicePostCallParams(lead: NormalizedBolnaPayload) {
...
export async function POST(req: NextRequest) {
  ...
  let eventKey = `bolna:ignored:${Date.now()}:${Math.random().toString(36).slice(2, 8)}`;

  try {
    const requiredSecret = process.env.BOLNA_WEBHOOK_SECRET?.trim();
    if (requiredSecret) {
      const tokenHeader =
        req.headers.get("x-bolna-token") ||
    ...
    const lead = normalizeBolnaPayload(parsedBody);
    if (!lead) {
      const ignoredRows = await db
        ...
        .values({
          ...
          errorMessage: "Ignored payload: unable to normalize Bolna webhook body",
        })
    ...
    if (!isTerminalBolnaStatus(lead.callStatus)) {
      return NextResponse.json({
        success: true,
        skipped: true,
        eventKey,
    ...
    return NextResponse.json({ error: "Bolna webhook processing failed", eventKey }, { status: 500 });
  }
}
