...
const practoWidgetId = process.env.NEXT_PUBLIC_PRACTO_WIDGET_ID?.trim() || "1e33a7c8d859e229";
const practoBookingUrl =
  process.env.NEXT_PUBLIC_PRACTO_BOOKING_URL?.trim() ||
              ...
              <div className="flex flex-col gap-5 md:flex-row md:items-start md:justify-between">
                <div>
                  ...
                  <p className="mt-2 max-w-lg text-sm leading-6 text-white/85">
                    Doctor context, trust framing and fallback actions stay in Santaan&apos;s voice even if the embedded
                    booking UI itself has limited styling options.
                  </p>
                </div>
            ...
            <div className="mt-4 rounded-[24px] border border-santaan-teal/10 bg-[#fcfaf7] p-4 md:p-5">
              <div className="mb-4 flex items-start gap-3 rounded-2xl bg-white px-4 py-3 shadow-sm">
                ...
                <p className="text-sm leading-6 text-santaan-teal/80">
                  Confidential fertility consultation. If the widget is unavailable on a device, patients can still
                  continue on WhatsApp or by phone without dropping intent.
                </p>
              </div>
              ...
              {practoWidgetId ? (
                <div className="rounded-[20px] border border-santaan-teal/10 bg-white px-6 py-10 shadow-inner">
                  {widgetStatus === "fallback" ? (
                    <div className="mx-auto max-w-md rounded-2xl border border-amber-200 bg-amber-50 px-5 py-6 text-center">
                      ...
                      <p className="mt-2 text-sm leading-6 text-santaan-teal/75">
                        The embedded widget did not load on this page. Patients can continue with a direct Practo page
                        if available, or use WhatsApp and phone support right away.
                      </p>
                      <div className="mt-5 flex flex-col justify-center gap-3 sm:flex-row">
                        ...
                        <a
                          ...
                          rel="noopener noreferrer"
                          className={cn(
                            buttonVariants({ variant: practoBookingUrl ? "outline" : "default", size: "lg" }),
                            "w-full sm:w-auto",
                          )}
                        >
                          <MessageCircle className="mr-2 h-5 w-5" />
                  ...
                  {widgetStatus === "loading" ? (
                    <p className="mt-4 text-center text-sm text-santaan-teal/65">Loading Practo booking...</p>
                  ) : null}
                </div>
              ) : (
                <div className="rounded-[20px] border border-dashed border-santaan-teal/20 bg-white px-6 py-10 text-center">
                  ...
                  <p className="mx-auto mt-3 max-w-xl text-sm leading-6 text-santaan-teal/75">
                    Add `NEXT_PUBLIC_PRACTO_WIDGET_ID` to render the live booking embed inside this wrapper. Until then,
                    users can book through the backup conversion paths below.
                  </p>
